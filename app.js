...
let scores = {
    highscore: [50000,40000,30000,20000,17792,9000,8000,7000,6000,
        5000,4000,3000,2000,1000,900,800,700,600,500,200],
highscoreNames: ['computer','computer','computer','computer','super big boy','computer','computer',
...
let gameState = {
    ...
    Death: new Audio('./pictures/Death.mp3'),
    Eat: new Audio('./pictures/Eat.mp3'),
    ...
    menu(){
        ...
        if(this.timesrun <= 0){
            ...
            class = 'name' type = "text" value =${this.playerName}>
            <button type="button" onclick= "gameState.game()"  class = "play" type = "submit">Play</button>
            `
            }else{
                startText = `
                <input placeholder ='PlayerName'
    ...
    game(){
        ...
        if(!gameState.firstRun){gameState.loseCheck();}
            if(gameState.end == true){
                ...
                gameState.gameInterval = 0;
                console.log('interval', gameState.gameInterval);
                return;
                }
                gameState.tick();
        }, 100);
    ...
    recordScore(){
        ...
        for (let k = 0; k < scores.highscore.length; k++){
            if(scores.highscore[k] < this.score){
                scores.highscore[k] = this.score
        ...
        for (let i = 0; i < scores.highscore.length; i++){
            if (scores.highscore[i] > storedScores.highscore[i]){
                storedScores.highscore[i] = scores.highscore[i];
                storedScores.highscoreNames[i] = scores.highscoreNames[i]
                break;
            }
        }
        ...
        for (let j = 0; j < 20; j++){
            ...
            tab.innerHTML = `${storedScores.highscoreNames[j]}: ${storedScores.highscore[j]}`
            score.appendChild(tab);
        }
        let highscores_serialized =JSON.stringify(storedScores)
